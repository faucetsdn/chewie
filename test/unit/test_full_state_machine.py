...
def check_counters(
    _func=None,
    *,
    expected_auth_counter=0,
    expected_failure_counter=0,
    expected_logoff_counter=0
):
    ...
    def decorator_check_counters(func):
        def wrapper(self):
            ...
            self.assertEqual(
                self.logoff_counter, start_logoff_counter + expected_logoff_counter
            )
            return ret
...
class FullStateMachineStartTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sm = FullEAPStateMachine(
            self.eap_output_queue,
    ...
    def auth_handler(
        self, client_mac, port_id_mac, *args, **kwargs
    ...
    def test_md5_challenge_request(self):
        ...
        self.sm.event(
            EventRadiusMessageReceived(eap_message, State.create(b"random state"))
        )
    ...
    def test_md5_challenge_response(self):
        ...
        self.assertIsInstance(
            self.radius_output_queue.get_nowait()[0], Md5ChallengeMessage
        )
    ...
    def test_discard(self):
        ...
        message = IdentityMessage(
            self.src_mac, message.message_id + 10, Eap.RESPONSE, "host1user"
        )
        self.sm.event(EventMessageReceived(message, self.PORT_ID_MAC))
        self.assertEqual(self.sm.state, self.sm.IDLE)
    ...
    def test_ttls_request(self):
        ...
        self.sm.event(
            EventRadiusMessageReceived(message, State.create(b"more random state"))
        )
        self.assertEqual(self.sm.state, self.sm.IDLE2)
        self.assertEqual(self.eap_output_queue.qsize(), 1)
        self.assertEqual(self.radius_output_queue.qsize(), 1)
        ...
        self.sm.event(
            EventRadiusMessageReceived(message, State.create(b"some more random state"))
        )
        self.assertEqual(self.sm.state, self.sm.IDLE2)
