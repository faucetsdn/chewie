...
class MessageParserTestCase(unittest.TestCase):
    ...
    def test_md5_challenge_request_message_parses(self):  # pylint: disable=invalid-name
        ...
        self.assertEqual(Eap.REQUEST, message.code)
        self.assertEqual(
            bytes.fromhex("824788d693e2adac6ce15641418228cf"), message.challenge
        )
        self.assertEqual(b"", message.extra_data)

    def test_md5_challenge_response_message_parses(
        self,
        ...
        self.assertEqual(Eap.RESPONSE, message.code)
        self.assertEqual(
            bytes.fromhex("3a535f0ee8c6b34fe714aa7dad9a0e15"), message.challenge
        )
    ...
    def test_success_message_packs(self):
        ...
        message = SuccessMessage(
            src_mac=MacAddress.from_string("00:19:06:ea:b8:8c"), message_id=255
        )
        packed_message = MessagePacker.ethernet_pack(
            message,
            MacAddress.from_string("00:19:06:ea:b8:8c"),
    ...
    def test_ttls_message_parses(self):
        packed_message = bytes.fromhex(
            "000000111101444444444444888e" "01000006016900061520"
        )
        message = MessageParser.ethernet_parse(packed_message)[0]
    ...
    def test_legacy_nak_message_parses(self):
        packed_message = bytes.fromhex(
            "0180c2000003000000111101888e" "01000006026800060315"
        )
        message = MessageParser.ethernet_parse(packed_message)[0]
    ...
    def test_radius_packs_with_nas_port(self):
        packed_message = bytes.fromhex(
            "01bf00610123456789abcdeffedcba9876543210010a62656e62757274741f1361613a62623a63633a64643a65653a66660506000002a14f18021500160410824788d693e2adac6ce15641418228cf50121139bd192c46fe6d2a937d9573311b70"
        )  # pylint: disable=line-too-long
    ...
    def test_radius_packs_basic(self):
        ...
        packed_message = bytes.fromhex(
            "01bf005b0123456789abcdeffedcba9876543210010a62656e62757274741f1361613a62623a63633a64643a65653a66664f18021500160410824788d693e2adac6ce15641418228cf5012caadc1c7a3be07fe63fdf83a59ed18c2"
        )  # pylint: disable=line-too-long
