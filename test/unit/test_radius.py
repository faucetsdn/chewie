...
from chewie.radius import (
    Radius,
    RadiusAccessAccept,
    RadiusAttributesList,
    InvalidResponseAuthenticatorError,
    RadiusAccessChallenge,
    RadiusAccessRequest,
)
...
class RadiusTestCase(unittest.TestCase):
    def test_radius_access_request_parses(self):
        ...
        self.assertEqual(
            binascii.hexlify(message.authenticator), b"982a0ba06d3557f0dbc8ba6e823822f1"
        )
        ...
        self.assertEqual(
            msg_attr.find(CalledStationId.DESCRIPTION).data(), "44-44-44-44-44-44:"
        )
        ...
        self.assertEqual(
            msg_attr.find(ConnectInfo.DESCRIPTION).data(), "CONNECT 0Mbps 802.11b"
        )
        self.assertEqual(
            msg_attr.find(AcctSessionId.DESCRIPTION).data(), "C648004A9C905579"
        )
        ...
        self.assertEqual(
            binascii.hexlify(msg_attr.find(MessageAuthenticator.DESCRIPTION).data()),
    ...
    def test_radius_access_accept_packs(self):
        ...
        attr_list = list()
        attr_list.append(
            VendorSpecific.create(bytes.fromhex("00003039010973747564656e74"))
        )
        ...
        access_accept = RadiusAccessAccept(
            1, bytes.fromhex("02970aff2ef0700780f70848e90d2410"), attributes
        )
    ...
    def test_corrupted_packets(self):
        ...
        try:
            ...
            self.assertIsInstance(
                exception.__cause__, InvalidResponseAuthenticatorError
            )
    ...
    def test_radius_access_challenge_parses(self):
        ...
        self.assertEqual(eap_msg.message_id, 2)
        self.assertEqual(
            binascii.hexlify(eap_msg.challenge), b"74d3db089b727d9cc5774599e4a32a29"
        )
        self.assertEqual(
            binascii.hexlify(msg_attr.find(MessageAuthenticator.DESCRIPTION).data()),
        ...
        self.assertEqual(
            binascii.hexlify(msg_attr.find(State.DESCRIPTION).data()),
    ...
    def test_radius_access_challenge_ttls_parses(self):
        ...
        self.assertEqual(
            binascii.hexlify(message.authenticator), b"54dbc73332c00c0347fc4b462d1811a7"
        )
        msg_attr = message.attributes
        self.assertEqual(len(msg_attr.attributes), 3)
        eap_msg = msg_attr.find(EAPMessage.DESCRIPTION).data()
        ...
        self.assertEqual(
            binascii.hexlify(eap_msg.extra_data),
            b""
    ...
    def test_parse_illegal_radius_datatype_lengths(self):
        self.assertRaises(
            MessageParseError, Integer.parse, (1500).to_bytes(3, byteorder="big")
        )
        self.assertRaises(
            MessageParseError, Integer.parse, bytes.fromhex("01234567890123456789")
        )

        self.assertRaises(MessageParseError, Enum.parse, bytes.fromhex("000002"))
        self.assertRaises(
            MessageParseError, Enum.parse, bytes.fromhex("01234567890123456789")
        )
    ...
    def test_radius_extract_attributes(self):
        ...
        attributes = []
        attributes_data = bytes.fromhex(
            ""
        )
        attributes_to_concat = {}
        RadiusAttributesList.extract_attributes(
            attributes_data, attributes, attributes_to_concat
        )
        ...
        attributes = []
        attributes_data = bytes.fromhex(
            ""
        )
        attributes_to_concat = {}
        RadiusAttributesList.extract_attributes(
            attributes_data, attributes, attributes_to_concat
    ...
    def test_concat_when_length_multiple_of_max_data_length(self):
        expected_packed = bytes.fromhex(
            ""
        )
        big_concat_bytes = bytes.fromhex(
            ""
        )
        concat = Concat(bytes_data=big_concat_bytes)
        packed = concat.pack(EAPMessage.TYPE)
        self.assertEqual(expected_packed, packed)

    def radius_user_password_encrypt(
        self, expected_password, secret, req_authenticator, expected_ciphertext
    ):
        ...
        self.assertEqual(
            ...
            "RADIUS User-Password encryption not equal %s : %s".format(
                binascii.hexlify(expected_ciphertext), binascii.hexlify(ciphertext)
            ),
        )
    ...
    def test_radius_user_password_short_encrypt(self):
        ...
        return self.radius_user_password_encrypt(
            expected_password, secret, req_authenticator, expected_ciphertext
        )
    ...
    def test_radius_user_password_long_encrypt_decrypt(self):
        ...
        expected_ciphertext = bytes.fromhex(
            "7654d755dffbb0d50ef7768e071ab01b5a371ee79883f81209ff482b6556d1fa"
        )
