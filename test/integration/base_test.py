...
def auth_handler(
    address, group_address, *args, **kwargs
):  # pylint: disable=missing-docstring
    logger = logging.getLogger("CHEWIE")
    logger.info(
        "Authentication successful for address %s on port %s",
        str(address),
        str(group_address),
    )
...
def failure_handler(address, group_address):  # pylint: disable=missing-docstring
    logger = logging.getLogger("CHEWIE")
    logger.info(
        "Authentication failed for address %s on port %s",
        str(address),
        str(group_address),
    )
...
def logoff_handler(address, group_address):  # pylint: disable=missing-docstring
    logger = logging.getLogger("CHEWIE")
    logger.info(
        "Logoff successful for address %s on port %s", str(address), str(group_address)
    )
...
class BaseTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        HANDLER_COUNTS = {"auth_handler": 0, "logoff_handler": 0, "failure_handler": 0}
    ...
    def open_logs(self):
        ...
        self.current_log_dir = (
            tempfile.mkdtemp(prefix="chewie-" + self.test_name + "-", dir="/tmp/logs")
            + "/"
        )
        print("Logging test results in {}".format(self.current_log_dir))
        print(os.path.join(self.current_log_dir + "wpa_supplicant.log"))
        self.freeradius_log = open(
            os.path.join(self.current_log_dir, "freeradius.log"), "w+"
        )
        self.wpa_supplicant_log = open(
            os.path.join(self.current_log_dir + "wpa_supplicant.log"), "w+"
        )
    ...
    def start_radius(self):
        ...
        return self.run_command_and_detach(
            "freeradius -X -l stdout", self.freeradius_log
        )
    ...
    def check_output(self, **kwargs):
        ...
        if chewie_requirements:
            for requirement in chewie_requirements:
                assert (
                    requirement in chewie_log
                ), "Unable to find {} in chewie logs".format(
                    requirement,
                )
        ...
        if currentframe:
            assert (
                "Authentication successful" in chewie_log
            ), "Authentication failed for {}".format(currentframe.f_back.f_code.co_name)
    ...
    def prepare_freeradius():
        ...
        try:
            ...
            file_map = {
                ...
                chewie_rad_dir
                + "default/inner-eap": radius_config_base
                + "mods-available/",
