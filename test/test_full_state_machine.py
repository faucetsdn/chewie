...
from chewie.message_parser import EapolStartMessage, IdentityMessage, SuccessMessage, \
    FailureMessage, EapolLogoffMessage, GenericMessage
...
class FullStateMachineStartTestCase(unittest.TestCase):
    ...
    def test_md5_challenge_request(self):
        ...
        eap_message = GenericMessage(self.src_mac, 2, Eap.REQUEST, Eap.MD5_CHALLENGE,
                                          extra_data=
        ...
        self.assertEqual(self.eap_output_queue.qsize(), 1)
        output = self.eap_output_queue.get_nowait()[0]
        self.assertIsInstance(output, GenericMessage)
    ...
    def test_md5_challenge_response(self):
        ...
        message = GenericMessage(self.src_mac, 2, Eap.RESPONSE, Eap.MD5_CHALLENGE,
                                      extra_data=
        ...
        self.assertEqual(self.eap_output_queue.qsize(), 0)
        self.assertEqual(self.radius_output_queue.qsize(), 1)
        self.assertIsInstance(self.radius_output_queue.get_nowait()[0], GenericMessage)
    ...
    def test_discard2(self):
        ...
        message = GenericMessage(self.src_mac, request.message_id + 10, Eap.RESPONSE,
                                 Eap.MD5_CHALLENGE,
                                 extra_data=build_byte_string("3a535f0ee8c6b34fe714aa7dad9a0e15") +
                                      b"host1user")
        self.sm.event(EventMessageReceived(message, None))
        self.assertEqual(self.sm.currentState, self.sm.IDLE2)
        self.assertEqual(self.eap_output_queue.qsize(), 0)
    ...
    def test_ttls_request(self):
        ...
        message = GenericMessage(self.src_mac, 2, Eap.RESPONSE, Eap.LEGACY_NAK,
                                 (21).to_bytes(1, 'big'))
        self.sm.event(EventMessageReceived(message, None))
        self.assertEqual(self.sm.currentState, self.sm.AAA_IDLE)
        self.assertEqual(self.eap_output_queue.qsize(), 0)
        ...
        message = GenericMessage(self.src_mac, 3, Eap.REQUEST, Eap.TTLS,
                                 0x20.to_bytes(1, 'big') + b'')
        self.sm.event(EventRadiusMessageReceived(message, None))
        self.assertEqual(self.sm.currentState, self.sm.IDLE2)
        self.assertEqual(self.eap_output_queue.qsize(), 1)
        self.assertEqual(self.radius_output_queue.qsize(), 1)

        message = GenericMessage(self.src_mac, 3, Eap.RESPONSE, Eap.TTLS,
                                 0x00.to_bytes(1, 'big') +
        ...
        message = GenericMessage(self.src_mac, 4, Eap.REQUEST, Eap.TTLS,
                                 0x00.to_bytes(1, 'big') + build_byte_string(
